...
# Load sounds
background_music = pygame.mixer.Sound("glossy-168156.wav")
pop_sound = pygame.mixer.Sound("pick-92276.wav")

# Play background music
background_music.play(-1)  # -1 loops indefinitely
...
def create_bubbles(num_bubbles):
    for _ in range(num_bubbles):
        ...
        bubble = Bubble(color, radius)
        bubble.rect.x = random.randint(0, SCREEN_WIDTH - radius * 2)
        bubble.rect.y = random.randint(0, SCREEN_HEIGHT - radius * 2)
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # Check if clicked bubble
            pos = pygame.mouse.get_pos()
            clicked_bubbles = [sprite for sprite in all_bubbles if sprite.rect.collidepoint(pos)]
            if clicked_bubbles:
                # Play pop sound
                pop_sound.play()
                # Remove clicked bubble
                all_bubbles.remove(clicked_bubbles)
                # Add new bubbles
                create_bubbles(1)
